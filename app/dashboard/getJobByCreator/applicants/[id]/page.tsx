...
const Applicants = () => {
  ...
  const handleAccept = async (jobSeekerId: any) => {
    try {
      ...
      successToast(res.message || "Applicant Accepted successfully");
    } catch (error: any) {
      const errorMessage = error?.data?.message || "Failed to Accept applicant";
  ...
  const handleReject = async (jobSeekerId: any) => {
    try {
      ...
      const errorMessage = error?.data?.message || "Failed to reject applicant";
      errorToast(errorMessage);
      console.error("Reject error:", error);
    }
  };
  ...
  return (
    <div className="w-full">
      {applicants?.data?.data.map((val: any, i: any) => (
        <div key={i} className="grid grid-cols-4 m-4 border border-slate-200 ">
          <div className="col-span-2 flex justify-evenl items-start m-2 space-x-8">
            ...
            <span className={`${val.status==="accepted"?"text-green-700":"text-red-700"}`}>{val.status}</span>
            {val.status==="accepted" && <span className="bg-green-600 w-4 h-4 rounded-full"></span>}
          ...
          <div className="col-span-2 flex justify-end items-center space-x-6">
            ...
            onClick={() => handleAccept(val.applicant._id)}
